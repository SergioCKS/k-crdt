/* tslint:disable */
/* eslint-disable */
export const memory: WebAssembly.Memory;
export function __wbg_uid_free(a: number): void;
export function uid_new(): number;
export function uid_fromString(a: number, b: number): number;
export function uid_toString(a: number, b: number): void;
export function uid_serialize(a: number, b: number): void;
export function uid_deserialize(a: number, b: number): number;
export function generateId(a: number): void;
export function createBoolRegister(a: number, b: number, c: number): void;
export function __wbg_browserhlc_free(a: number): void;
export function browserhlc_new(): number;
export function browserhlc_getOffset(a: number, b: number): void;
export function browserhlc_setOffset(a: number, b: number, c: number): void;
export function browserhlc_serialize(a: number, b: number): void;
export function browserhlc_deserialize(a: number, b: number): number;
export function browserhlc_generateTimestamp(a: number): number;
export function browserhlc_updateWithTimestamp(a: number, b: number): number;
export function __wbg_timestamp_free(a: number): void;
export function timestamp_toString(a: number, b: number): void;
export function timestamp_serialize(a: number, b: number): void;
export function timestamp_deserialize(a: number, b: number): number;
export function __wbindgen_malloc(a: number): number;
export function __wbindgen_realloc(a: number, b: number, c: number): number;
export function __wbindgen_add_to_stack_pointer(a: number): number;
export function __wbindgen_free(a: number, b: number): void;
export function __wbindgen_exn_store(a: number): void;
